...
class WLButton: UIButton {
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch : AnyObject in touches {
            ...
            let maxCenterY = (WLWindowHeight - UIScreen.tabbarHeight - UIScreen.statusHeight - UIScreen.tabbarSafeBottomMargin) - self.frame.size.height / 2
            let minCenterY  = self.frame.size.height / 2 + UIScreen.statusHeight
            if self.center.y < minCenterY{
                let x : CGFloat = self.center.x
                
                let y : CGFloat = minCenterY
                self.center = CGPoint(x: x, y: y)
            }else if self.center.y > maxCenterY{
                
                let x : CGFloat = self.center.x
                
                let y : CGFloat = maxCenterY
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if self.center.x >= (self.superview?.frame.size.width)! / 2 {
            ...
            self.frame = CGRect(x: WLWindowWidth - self.width, y: self.center.y - self.height/2, width: self.width, height: self.height)
            ...
            self.frame = CGRect(x: 0, y: self.center.y - self.height/2, width: self.width, height: self.height)
